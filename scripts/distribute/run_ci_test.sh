...
####################################
export paddle=$1
export nlp_dir=/paddle/xuexixi/PaddleNLP
mkdir -p /paddle/xuexixi/PaddleNLPcase_logs
export log_path=/paddle/xuexixi/PaddleNLP/case_logs
export case_list=()
...
if [[ ${#case_list[*]} -ne 0 ]];then
    ...
    if [[ $(contain_case gpt-3_dygraph ${case_list[@]}; echo $?) -eq 1 ]];then
        ...
        bash /paddle/xuexixi/PaddleNLP/scripts/distribute/ci_case_dy.sh gpt_case_list_dygraph $FLAGS_install_deps $FLAGS_download_data
